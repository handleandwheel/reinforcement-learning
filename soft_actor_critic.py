...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim: int, act_dim: int, hidden_size: List[int]):
        ...
        self._net = ActorMLP(obs_dim, hidden_size, hidden_size[-1])
        self._mu_layer = nn.Linear(hidden_size[-1], self._act_dim)
        self._log_std_layer = nn.Linear(hidden_size[-1], self._act_dim)
    
    def forward(self, obs: torch.Tensor, with_log_prob: bool, deterministic: bool = False):
        """
        return the value of log probability given obs and act
        """
        net_out = self._net(obs)
        ...
        if with_log_prob:
            log_pi = pi.log_prob(action).sum(axis=-1)
            log_pi -= (2 * (torch.log(torch.tensor(2)) - action - F.softplus(-2 * action))).sum(axis=1)
        else:
            log_pi = None
...
class SAC(object):
    ...
    def __init__(self, env: Env, actor_hidden: List[int], critic_hidden: List[int], policy_lr: float, value_lr: float, buffer_size: int, reward_scale: float, device: str) -> None:
        ...
        self._pi_net = Actor(self._obs_dim, self._act_dim, actor_hidden)
        ...
        self._pi_optimizer = Adam(self._pi_net.parameters(), lr=policy_lr)
    ...
    def eval(
        ...
        while not done and time_step < max_step:
            with torch.no_grad():
                act, _ = self._pi_net(torch.tensor(obs, dtype=torch.float32), False, True)
    ...
    def train(
        self, 
        epochs: int = 10000,
        update_period: int = 1, # how many epoch per update round
        update_num: int = 200, # how many update per update round
        batch_size: int = 128,
        start_after: int = 1000, # how many epoch before starting training
        warm_up_epochs: int = 100,
        rho = 0.995,
        gamma: float = 0.99,
        alpha: float = 0.02, # this parameter should be coupled with the magnitude of reward
        eval_freq: int = 200,
        max_step: int = 200
    ):
        ...
        for k in range(epochs):
            ...
            while not done:
                if k >= warm_up_epochs:
                    with torch.no_grad():
                        act, _ = self._pi_net(torch.tensor(obs, dtype=torch.float32), False)
                    act = act.numpy()
                else:
            ...
            if (k + 1) % update_period == 0 and k > start_after:
                ...
                for i in range(update_num):
                    ...
                    with torch.no_grad():
                        ...
                        next_acts, log_pi = self._pi_net(next_obss, True)
                        
                        q1 = self._q1_target(next_obss, next_acts)
                        q2 = self._q2_target(next_obss, next_acts)
                        targets = rews + gamma * (1 - dones) * (torch.min(q1, q2) - alpha * log_pi)
                    ...
                    for q in self._q2_net.parameters():
                        q.requires_grad = False
                        
                    self._pi_optimizer.zero_grad()
                    acts, log_pi = self._pi_net(obss, True)
                    q1 = self._q1_net(obss, acts)
                    q2 = self._q2_net(obss, acts)
                    pi_loss = (alpha * log_pi - torch.min(q1, q2)).mean()
                    pi_loss.backward()
